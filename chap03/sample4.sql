...
-- ------------------------------------------------------
SELECT
    -- sysdate (X), systimestamp (X) => 
    -- current_date (OK), current_timestamp (OK)
...
SELECT
    -- 날짜 데이터 -> 문자 데이터로 강제 형변환
    -- to_char(current_date, ' YYYY "년" MM "월" DD "일" ') 날짜
    to_char(current_date, 'YYYY "년" MM "월" DD "일"') 날짜
FROM
    dual;
...
SELECT
    ...
    to_char(12346789, '099999999999')    AS "실험1",    -- 실험1
...
SELECT
    ...
    length('$') 달러길이,
    length(',') 쉼표길이,
    length( to_char(salary, '$999,999') ) 길이,
