...
-- 나. 여러 테이블의 조인조건을, WHERE 절에 명시하지 않고,
...
FROM
    ...
    -- employees, departments in Oracle Catesian Product 집합연산
...
SELECT
    ...
    department_name
FROM
    -- 자연조인(Naturla Join) == 동등조인(Equi Join) + 중복컬럼제거
...
SELECT
    ...
    -- t1.department_id            -- XX: 공통컬럼은 수식어(Qualifier)를 가져서는 안됨
    -- employees.department_id     -- XX: 공통컬럼은 수식어(Qualifier)를 가져서는 안됨
...
FROM
    ...
    -- USING절의 한계: 무조건 Equi Join -> Non-Equi Join 수행불가
