...
-- ------------------------------------------------------
-- 커밋(COMMIT)  : DML 문장에 의한 DB의 변경내역을 영구적으로 DB파일에 저장하는 기능
-- 롤백(ROLLBACK): DML 문에 의한 변경이 발생하기 **이전상태로 회귀** 시킴
-- DML 1개 문장이 실행완료될 때마다, 자동으로 COMMIT 문을 수행시키는 기능

SHOW AUTOCOMMIT;        -- 현재 DB 연결세션에서, 자동커밋을 설정을 보여줌

SET AUTOCOMMIT ON;      -- 현재 DB 연결세션에 대해서, 자동커밋을 ON 시킴
SET AUTOCOMMIT OFF;     -- 현재 DB 연결세션에 대해서, 자동커밋을 OFF 시킴
...
BEGIN       -- To begin a transaction
    ...
    -- 3. 특정 컬럼에 묵시적으로 널(null, 결측치) 저장:
    --    (1) 묵시적 방법(Implicitly)
    INSERT INTO dept (deptno, dname)
    VALUES (70, '인사');

    -- 4. 특정 컬럼에 명시적으로 널(null, 결측치) 저장:
    --    (2) 명시적 방법(Explicitly)
    INSERT INTO dept (deptno, dname, loc)
    VALUES (80, '인사', NULL);

    -- 5. TCL: COMMIT 또는 ROLLBACK 문으로 구성됨
    COMMIT;         -- 위의 INSERT 문에 의한 변경을 영구적으로 저장 (***)
    -- ROLLBACK;    -- 위의 INSERT 문에 의한 변경이 발생하기 이전상태로 회귀시킴 (***)
...
--  Subquery;
-- ------------------------------------------------------
DROP TABLE mydept;  -- DDL (Data Definition Language) : 테이블삭제
