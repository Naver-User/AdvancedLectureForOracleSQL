...
--    b. 정렬을 위한 ORDER BY 절은 사용불가 (틀림 => 사용가능!)
--    c. 정렬 필요시, View 이용 검색쿼리에서 기술해야 함 (틀림 => 사용가능!)
--  * View 의 수정:
...
CREATE OR REPLACE VIEW emp_view (사번, 이름, 부서명, 부서번호) AS
SELECT
    empno,
    ename,
    d.dname,
...
CREATE OR REPLACE VIEW emp_view4 (부서, 총급여액) AS
SELECT
    deptno,
    sum(sal)
    -- sum(sal) AS 총급여액
...
CREATE OR REPLACE VIEW emp_view5 (부서, 사원수) AS
SELECT 
    deptno,
    -- count(*) 
    count(empno)
...
SELECT
    t2.department_name AS 부서,
    count(t1.employee_id) AS 사원수
FROM
    employees t1,
    departments t2
WHERE
    t1.department_id = t2.department_id     -- 조인조건(동등조인, Equi)
GROUP BY
    t2.department_name;
...
--      INSERT/UPDATE (DML) 가 가능하도록 제약조건 설정가능
...
WHERE
    deptno = 30
WITH CHECK OPTION;  -- 제약조건 추가하여, 뷰 수정
...
-- 더 쉽게: 뷰에 대한 DML작업이, 뷰를 만든 서브쿼리의 WHERE절의 조건을
--          만족(true)시키지 못하면(false), 그 DML 작업 불허

-- WITH CHECK OPTION 의 데이터 수정 불가
UPDATE emp_view6
SET 
    -- 즉, 아래 부서번호를 30 => 40으로 변경하는 것은,
    -- 뷰의 서브쿼리의 WHERE절의 조건을 false로 만들기 때문에, 
    -- 이런 DML 작업을 제약하는 것이 바로 WITH CHECK OPTION 제약조건임
    -- deptno = 40,        -- XX : WITH CHECK OPTION 제약조건 위배
